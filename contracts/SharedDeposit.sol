...
contract SharedDeposit is Ownable, ReentrancyGuard {
    ...
    bool public validatorsCreated = false;
    ...
    constructor(uint256 _numValidators, uint256 _adminFee, address _BETHTokenAddress) public {
        ...
        //pricePerShare = numValidators.mul(32).mul(1e18).add(adminFee).div((32).mul(1e18));
    }

    function maxValidatorShares() external view returns (uint256) {
        return costPerValidator.mul(numValidators).mul(1e18);
    }

    // USER INTERACTIONS
    function deposit() public payable nonReentrant {
        // input is whole, not / 1e18 , i.e. in 1 = 1 eth send

        //make sure max validator limit not yet reached
        // make sure its mintable
        // TODO: fix this check require(address(this).balance.add(msg.value) < uint256(32).mul(numValidators).mul(1e18));
        uint value = uint(msg.value);

        uint shareOfOne = value.div(32);
        uint valMinusAdmin = value.sub(adminFee.mul(shareOfOne));
        uint newShareTotal = curValidatorShares.add(valMinusAdmin);

        require(newShareTotal <= costPerValidator.mul(numValidators).mul(1e18), "Eth2Staker:deposit:Amount too large, not enough validators left");
        curValidatorShares = newShareTotal;
        adminFeeTotal = adminFeeTotal.add(adminFee.mul(shareOfOne));
        u.mint(msg.sender, valMinusAdmin);
    }

    function withdraw(uint256 amount) public nonReentrant {
        // convert to the same format as deposit for convenient?
        uint shareOfOne = amount.div(32);
        uint valMinusAdmin = amount.sub(adminFee.mul(shareOfOne));
        uint newShareTotal = curValidatorShares.sub(valMinusAdmin);
        ...
        require(u.balanceOf(msg.sender) >= amount, "Eth2Staker: Sender balance not enough");

        // Todo : check if deadline has passed or not
        // make sure there is money in the contract

        curValidatorShares = newShareTotal;
        adminFeeTotal = adminFeeTotal.sub(adminFee.mul(shareOfOne));
    ...
    function depositToEth2(bytes calldata pubkey,
        ...
        bytes32 deposit_data_root) external onlyOwner {
            uint256 amount = numValidators.mul(32000000000000000000);
